            ...
            <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-100">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-6">
                        ...
                        <textarea
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none h-24"
                    ...
                    <div className="mb-6">
                        <label className="block text-gray-700 font-medium mb-2">Departamentos Asignados</label>
                        <div className="border border-gray-300 rounded-lg p-3 max-h-48 overflow-y-auto space-y-2">
                            {allDepartments.length === 0 ? (
                                <p className="text-gray-400 text-sm">No hay departamentos configurados.</p>
                            ) : (
                        ...
                        <p className="text-xs text-gray-400 mt-1">{selectedDepartments.length} departamento(s) seleccionado(s)</p>
                    </div>

                    <div className="mb-6">
                        <label className="block text-gray-700 font-medium mb-2">Usuarios Asignados</label>
                        <div className="border border-gray-300 rounded-lg p-3 max-h-48 overflow-y-auto space-y-2">
                            {allUsers.length === 0 ? (
                                ...
                                allUsers.map(user => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            <span className="text-sm font-medium text-gray-800">{user.displayName || "Sin nombre"}</span>
                                            <span className="text-xs text-gray-500 ml-2">{user.email}</span>
                                        </div>
                                        {user.id === currentUser.uid && (
                                            <span className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full ml-auto">Tú</span>
                                        )}
                                    </label>
                                ))
                            )}
                        </div>
                        <p className="text-xs text-gray-400 mt-1">{selectedUsers.length} usuario(s) seleccionado(s)</p>
                    </div>
